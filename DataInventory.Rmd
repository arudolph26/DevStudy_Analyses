...
data.inv$care <- check_complete(q.data, "subj_id", matrix(rep(care_qs,4),ncol=4), data.inv, "subjects")
...
```{r}
data.inv$iq <- check_complete(q.data, "subj_id", matrix(rep("fsiq2",4),ncol=4), data.inv, "subjects")

data.inv$vocab <- check_complete(q.data, "subj_id", matrix(rep("vocab_raw",4),ncol=4), data.inv, "subjects")

data.inv$matrea <- check_complete(q.data, "subj_id", matrix(rep("mr_raw",4),ncol=4), data.inv, "subjects")
```
...
```{r}
grep('basct', names(q.data), value=T)
grep('basca', names(q.data), value=T)
grep('basc[0-999]$', names(q.data), value=T)
```

`q.data` should have data for: basc
