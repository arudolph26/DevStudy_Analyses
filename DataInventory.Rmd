...
This should only be true for `subj_id` > 100000
...
data.inv$duq <- check_complete(q.data, "subj_id", matrix(rep(duq_qs,4),ncol=4), data.inv, "subjects")
...
data.inv$matrea <- check_complete(q.data, "subj_id", ist("mr_raw", "mr_raw", "mr_raw", "mr_raw"), data.inv, "subjects")
```
...
```{r}
bart.mat.list <- read.table(pipe('ssh -l zenkavi ls5.tacc.utexas.edu "ls  /corral-repl/utexas/poldracklab/users/zenkavi/dev_study/data/behav/bart/*.mat"'))

bart.mat.list$V1 <- sub("/corral-repl/utexas/poldracklab/users/zenkavi/dev_study/data/behav/bart/", "",bart.mat.list$V1)
...
```{r}
mg.csv.list <- read.table(pipe('ssh -l zenkavi ls5.tacc.utexas.edu "ls  /corral-repl/utexas/poldracklab/users/zenkavi/dev_study/data/behav/machine_game/*.csv"'))

mg.csv.list$V1 <- sub("/corral-repl/utexas/poldracklab/users/zenkavi/dev_study/data/behav/machine_game/ProbLearn", "", mg.csv.list$V1)

mg.csv.list$subjects <- as.numeric(sub(".csv", "", mg.csv.list$V1))
...
Some form of DICOMS for each subject

```{r}

mg.dicom.zip.list <- read.table(pipe('ssh -l zenkavi ls5.tacc.utexas.edu "ls  /corral-repl/utexas/poldracklab/openfmri/inprocess/ds054_shelfins/ds054_dicom/01_DICOM_zipfiles_from_XNAT/*.zip"'))

mg.dicom.zip.list$V1 <- sub("/corral-repl/utexas/poldracklab/openfmri/inprocess/ds054_shelfins/ds054_dicom/01_DICOM_zipfiles_from_XNAT/", "", mg.dicom.zip.list$V1)

gsub("([0-9]+).*$", "\\1", mg.dicom.zip.list$V1)

unique(na.omit(as.numeric(unlist(strsplit(mg.dicom.zip.list$V1, "[^0-9]+")))))

a <- na.omit(as.numeric(unlist(strsplit(mg.dicom.zip.list$V1, "[^0-9]+"))))
```
