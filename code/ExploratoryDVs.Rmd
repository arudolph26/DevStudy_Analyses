...
tmp = machine_game_data_clean %>%
  group_by(Sub_id, facet_labels) %>%
  do(get_obs_var_ev(.))%>% 
  mutate(play1_pass0 = ifelse(Response == "play", 1, 0))
```
...
```{r warning=FALSE, message=FALSE}
tmp %>%
  ggplot(aes(obs_var, correct1_incorrect0))+
  geom_line(aes(group = Sub_id, col= age_group),stat='smooth', method = 'glm', method.args = list(family = "binomial"), se = FALSE, alpha=0.2)+
  geom_line(aes(col= age_group),stat='smooth', method = 'glm', method.args = list(family = "binomial"), se = FALSE, alpha=1, size=2)+
  facet_wrap(~facet_labels, scales='free')+
  theme_bw()+
...
```{r}
# This model is not really interesting since we are interested in the relative effect of observed EV and var across age groups
# summary(glmer(correct1_incorrect0 ~ scale(obs_var)+scale(obs_ev)+age_group+facet_labels+(1|Sub_id),              family = binomial(link=logit), data = tmp))

#Convergence problems begin even here
# summary(glmer(correct1_incorrect0 ~ scale(obs_var)+scale(obs_ev)+age_group+facet_labels+
...
# glmer(correct1_incorrect0 ~ scale(obs_var)+scale(obs_ev)+age_group+facet_labels+
