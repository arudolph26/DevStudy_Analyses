...
What direction are the learner group differences in?
...
RPEs from the averaging of exp_exp models seem to have both higher correlations with brain activity but also more variability in these correlations. Is this because these correlations can discriminate between learner/not-learner groups?

Two points from this graph:
...
The change of relationship between RPEs predicted by exp_exp models and brain activity have different profiles for learners and non-learners but for neither group is this relationship monotonous. Thus it is difficult to pinpoint a group difference that would map on to a cognitive process.
...
Overall I think the case for exp_exp models is stronger.
...
In terms of clarifying the confusing picture in the literature using exp models has pros and cons. Since they haven't been used in this line of reseach before the models are not directly comparable. But on the other hand they take into account a wealth of other literature on value processing and with a single parameter extend this class of models to account for known differences in value distortation particularly with respect to the doman (gain vs loss). It also provides an answer for the developmental difference in this new parameter which could provide a partial reason for why the previous results were not compatible. 