...
transform_remove_skew(data.frame(tmp))

#trying to figure out how to transform variables separately for each model so the plot depicting the parameter differences of all groups is more accurate since the models were run on transformed data and the current plot depits non-transformed parameter estimates
```
