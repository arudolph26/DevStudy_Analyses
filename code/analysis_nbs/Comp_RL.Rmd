...
best_sub_pars %>%
  ...
  summarise(mean_val = mean(value,na.rm=T),
            sem_val = sem(value)) %>%
  filter(x_axis %in% c("\u03b1, \u03b2, \u03b3_gain, \u03b3_loss", "\u03b1_gain, \u03b1_loss, \u03b2, \u03b3_gain, \u03b3_loss")) %>%
  # filter(x_axis %in% c("\u03b1, \u03b2, \u03b3_gain, \u03b3_loss")) %>%
...
# ggsave("RL_par_diffs_ab2gs.jpeg", device = "jpeg", path = fig_path, width = 2.8, height = 5, units = "in", dpi = 450)
ggsave("RL_par_diffs_2mods.jpeg", device = "jpeg", path = fig_path, width = 4.9, height = 5, units = "in", dpi = 450)
```

```{r}

summary(lm(xopt_alpha_pos ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta_Fix_exp_")))

summary(lm(xopt_alpha_neg ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta_Fix_exp_")))

summary(lm(xopt_beta ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta_Fix_exp_")))

##############

summary(lm(xopt_alpha_pos ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_Fix_")))

summary(lm(xopt_alpha_neg ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_Fix_")))

summary(lm(xopt_beta ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_Fix_")))

summary(lm(xopt_exp ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_Fix_")))

##############

summary(lm(xopt_alpha_pos ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_alpha_neg ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_beta ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_exp_pos ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_exp_neg ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_")))

##############

summary(lm(xopt_alpha ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_beta ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_exp_pos ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha-beta-exp_neg-exp_pos_Fix_")))

summary(lm(xopt_exp_neg ~ age_group, best_sub_pars %>% filter(model == "Fit_alpha-beta-exp_neg-exp_pos_Fix_")))
...
#   arrange(adult, teen)
age_diff_df %>%
  mutate(adult_adjust = p.adjust(adult), method="bonferroni")%>%
  filter(adult<0.05)
```
...
**- We have models that are on average at chance in predicting behavior.** 
...
ggsave("EZ_par_diffs.jpeg", device = "jpeg", path = fig_path, width = 9, height = 5, units = "in", dpi = 450)
```

```{r r warning=FALSE, message=FALSE}
ez_fits_playpass = read.csv(paste0(input_dir,"EZ_ddm_fits_play1_pass0_all.csv" ))

ez_fits_playpass = ez_fits_playpass %>% 
  rename(sub_id = X) %>%
  mutate(age_group = ifelse(sub_id<200000, "kid", ifelse(sub_id>200000 & sub_id<300000, "teen", "adult")),
         age_group = factor(age_group, levels = c("kid","teen","adult"))) %>%
  gather(key, value, -sub_id, -age_group) %>%
...
ggsave("EZ_par_diffs_play_pass.jpeg", device = "jpeg", path = fig_path, width = 9, height = 5, units = "in", dpi = 450)
```
...
ggsave("EZ_preds_comp_age.jpeg", device = "jpeg", path = fig_path, width = 9, height = 5, units = "in", dpi = 450)
```
...
choice_data %>%
  ...
  mutate(pred_pc = 1/(1+exp(-thresh*drift/0.1^2)),
         pred_correct1_incorrect0 = ifelse(pred_pc>0.5,1,0)) %>%
  mutate(correct_positive = ifelse(correct1_incorrect0 == 1 & pred_correct1_incorrect0 == 1, 1, 0),
         correct_negative = ifelse(correct1_incorrect0 == 0 & pred_correct1_incorrect0 == 0, 1, 0),
         false_positive = ifelse(correct1_incorrect0 == 0 & pred_correct1_incorrect0 == 1, 1, 0),
         false_negative = ifelse(correct1_incorrect0 == 1 & pred_correct1_incorrect0 == 0, 1, 0)) %>%
  group_by(age_group, machine) %>%
  summarise(sum_correct_positive = sum(correct_positive, na.rm=T),
            sum_correct_negative = sum(correct_negative, na.rm=T),
            sum_false_positive = sum(false_positive, na.rm=T),
            sum_false_negative = sum(false_negative, na.rm=T)) %>%
  gather(key, value, -age_group, -machine) %>%
  mutate(TChoice = ifelse(key %in% c("sum_correct_positive", "sum_false_negative"),1,0),
         PChoice = ifelse(key %in% c("sum_correct_positive", "sum_false_positive"), 1, 0)) %>%
  ggplot(mapping = aes(x = factor(TChoice), y = factor(PChoice))) +
  geom_tile(aes(fill = value), colour = "white") +
  ...
  xlab("True Choice")+
  ylab("Predicted Choice")+
  facet_grid(age_group~machine)+
  theme(legend.position = "none")

ggsave("EZ_confusion_matrices.jpeg", device = "jpeg", path = fig_path, width = 9, height = 5, units = "in", dpi = 450)
```
