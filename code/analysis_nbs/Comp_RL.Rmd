...
# #The ones I have 
# "LearningParams_Fit_alpha_neg-alpha_pos-beta-exp_Fix_All.csv"
# "LearningParams_Fit_alpha_neg-alpha_pos-beta-exp_neg-exp_pos_Fix_All.csv"
# "LearningParams_Fit_alpha-beta-exp_Fix_All.csv" 
# "LearningParams_Fit_alpha-beta-exp_neg-exp_pos_Fix_All.csv" 
# 
# #The ones I lack: exp=1/no exp
# "LearningParams_Fit_alpha-beta_Fix_All.csv"  
# 'LearningParams_Fit_alpha_neg-alpha_pos-beta_Fix_All.csv'
# 
# #I sub them with: exp = 0.5
# 'LearningParams_Fit_alpha-beta_Fix_exp_All.csv'
# 'LearningParams_Fit_alpha_neg-alpha_pos-beta_Fix_exp_All.csv'
```
...
```{r}
tmp = read.csv(paste0(input_dir, "LearningParams_Fit_alpha-beta-exp_neg-exp_pos_Fix_All.csv"))
```


```{r eval=FALSE}
for(cur_sub in unique(tmp$sub_id)){
  p = tmp %>%
    select(sub_id, Unnamed..0, contains('xopt'), neglogprob) %>%
    rename(fit_num = Unnamed..0) %>%
    group_by(sub_id) %>%
    mutate(min_col = factor(ifelse(neglogprob == min(neglogprob, na.rm=T),1,NA))) %>%
    gather(key, value, -sub_id, -fit_num, -neglogprob, -min_col) %>%
    filter(sub_id == cur_sub) %>%
    ggplot(aes(fit_num, value))+
    geom_line(aes(group=sub_id), alpha=0.5)+
    geom_point(aes(col=min_col))+
    facet_wrap(~key, scales="free")+
    theme(legend.position = "none")
  p_name = paste0(cur_sub, '_Fit_alpha-beta-exp_neg-exp_pos_Fix')
  ggsave(paste0(p_name, '.jpeg'), plot=p, device = 'jpeg', path = paste0(fig_path, 'pars'), units = "in", limitsize = FALSE)
}
