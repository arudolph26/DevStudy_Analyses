...
best_sub_pars %>%
  ...
  mutate(key = ifelse(key == "xopt_alpha", "\u03b1", ifelse(key == "xopt_beta", "\u03b2", ifelse(key == "xopt_exp", "\u03b3", ifelse(key == "xopt_alpha_pos", "\u03b1_gain", ifelse(key == "xopt_alpha_neg", "\u03b1_loss",ifelse(key == "xopt_exp_pos", "\u03b3_gain", ifelse(key == "xopt_exp_neg", "\u03b3_loss", ifelse(key == "xopt_lossave", "\u03bb", NA)))))))))%>%
...
ggsave("RL_par_diffs.jpeg", device = "jpeg", path = fig_path, width = 12, height = 5, units = "in", dpi = 450)
```
...
best_sub_pars %>%
  ...
  mutate(key = ifelse(key == "xopt_alpha", "\u03b1", ifelse(key == "xopt_beta", "\u03b2", ifelse(key == "xopt_exp", "\u03b3", ifelse(key == "xopt_alpha_pos", "\u03b1_gain", ifelse(key == "xopt_alpha_neg", "\u03b1_loss",ifelse(key == "xopt_exp_pos", "\u03b3_gain", ifelse(key == "xopt_exp_neg", "\u03b3_loss", ifelse(key == "xopt_lossave", "\u03bb", NA)))))))))%>%
    ggplot(aes(value))+
    geom_density(aes(fill=age_group), alpha=0.2)+
  facet_grid(x_axis~key, scales='free')+
  ylab("")+
  xlab("")+
  theme(panel.grid = element_blank(),
        legend.position = "none")

ggsave("RL_par_dists.jpeg", device = "jpeg", path = fig_path, width = 12, height = 5, units = "in", dpi = 450)
```
...
unique(all_mods_preds$model)

# What am I looking for?
# The correlation between the PE for each model
# The correlation will be different for each subject
# So first summarise and get the correlation for each subject
# The look at average correlation across subjects for each model
...
for(i in 1:(length(mods) - 1) ){
  ...
  for(j in 1:length(rem_mods)){
    cur_data = all_mods_pes %>% select(sub_id, mods[m], rem_mods[j])
