...
best_sub_pars %>%
  ...
  filter(grepl("\u03bb",key) == FALSE) %>%
    ...
    geom_bar(aes(fill=age_group, color=fct_brdr), stat="identity")+
...
ggsave("RL_par_diffs_nolamda.jpeg", device = "jpeg", path = fig_path, width = 12, height = 5, units = "in", dpi = 450)
```
...
best_sub_pars %>%
  ...
  mutate(key = ifelse(key == "xopt_alpha", "\u03b1", ifelse(key == "xopt_beta", "\u03b2", ifelse(key == "xopt_exp", "\u03b3", ifelse(key == "xopt_alpha_pos", "\u03b1_gain", ifelse(key == "xopt_alpha_neg", "\u03b1_loss",ifelse(key == "xopt_exp_pos", "\u03b3_gain", ifelse(key == "xopt_exp_neg", "\u03b3_loss", ifelse(key == "xopt_lossave", "\u03bb", NA)))))))))%>%
    ggplot()+
    # geom_density(aes(value, fill=age_group), alpha=0.2)+
  geom_boxplot(aes(age_group, value, fill=age_group))+
...
cor_df %>%
  ...
  mutate(cor_type = paste0(x_axis.x, "+", x_axis.y)) %>%
  group_by(cor_type) %>%
...
ggsave("PE_corrs.jpeg", device = "jpeg", path = fig_path, width = 7, height = 12, units = "in", dpi = 450)
```
## Imaging 
