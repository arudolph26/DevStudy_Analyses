...
Real-world risk was measures using two questionnaires with a total of 64 items. Not all of these had sufficient variance to be useful so filtered out those that could not be transformed successfully to have skewness <2 and applied hierarchical clustering on the remaining items. This resulted in three factors: Alcohol, smoking, social risk taking.

Distributions of 'real-world risk' factors:
...
```{r warning=FALSE, message=FALSE}
all_behavioral_data %>%
  select(bart_mean_rt, '-5,+495_sum_correct', smoking_scores, alcohol_scores) %>%
  gather(key, value, -smoking_scores, -alcohol_scores) %>% 
...
ggsave("Risk_cors.jpeg", device = "jpeg", path = fig_path, width = 9, height = 7, units = "in", dpi = 450)
