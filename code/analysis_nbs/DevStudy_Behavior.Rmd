...
fits = "LearningParams_Fit_alpha-beta-exp_neg-exp_pos_Fix_All.csv"
for(f in fits){
  ...
  data %>%
    ggplot(aes(xopt_alpha))+
    xlim(-0.25,2)+
    ylim(0,3)+
    geom_density(aes(group=factor(sub_id)), color="gray")+
    geom_density(fill="black")+
    theme(legend.position = "none")
  
  data %>%
    # filter(sub_id %in% c(100009, 200056, 408952)) %>%
    ggplot(aes(xopt_beta))+
    geom_density(aes(group=factor(sub_id)), color="gray")+
    geom_density(fill="black")+
    theme(legend.position = "none")+
    xlim(-1,3)+
