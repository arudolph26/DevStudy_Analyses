...
What affects whether a subject plays after experiencing a loss?
The magnitude of the loss they expired?
How long they think before playing?
...
machine_game_data_clean %>%
  ...
  theme(legend.title = element_blank())+
  xlab("Trial number")+
  ylab("Probability of playing following a loss")
```
...
summary(glmer(postloss_play1_pass0 ~ facet_labels+scale(Reaction_time)+age_group+(1|Sub_id), tmp, family="binomial"))
```
