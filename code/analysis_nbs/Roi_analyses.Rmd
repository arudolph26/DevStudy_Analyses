...
ggsave("ROI_kurtosis.jpeg", device = "jpeg", path = fig_path, width = 8, height = 3, units = "in", dpi = 450)    
```
...
In a second analysis  we examined whether the time series in an apriori l_vstr seed was similar to any other voxels' time series in 8 other subjective value network ROI spheres all taken from Barta et al., 2013.  
...
All ROIs correlate >0 with the lvstr seed throughout the whole task but there is no difference in these relationships depending on the learner group.
...
Notably these mean correlations between each ROI and l_vstr seem to vary more between participants. Is it suitable to be related to behavioral parameter estimates?
...
all_seed2seed %>%
  ...
  geom_errorbar(aes(ymin = mean_cor_val-sem_cor_val, ymax=mean_cor_val+sem_cor_val), position=position_dodge(width=.9), width=0.2)+
  facet_wrap(~seeds)+
  xlab("Run")+
