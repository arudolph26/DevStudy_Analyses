...
#for the l-vstr ROI there are
#81 voxels
#i can look at peak value
#or mean value
#the former might inflate an outlier
#the latter might wash out an effect
#so what if I look at each voxel separately?
#for each subject we have the HPE related form 81 voxels for each run
#does the distribution of "value" in each of the 81 voxels 
#value ~ run_num*learner, data= 1 per subject b for a given voxel
hpe_lvstr_regs = all_roi_pe_betas %>% 
  filter(regressor == "hpe" & roi == "l_vstr") %>%
...
#does runnum matter for any voxel?
#does the parameter estimate change across runs? No.
hpe_lvstr_regs %>%
  filter(term=="run_num") %>%
...
#Do the parameter estimates change differently from run to run for the two learner groups? For one voxel only
hpe_lvstr_regs %>%
  filter(term=="run_num:learnerNon-learner") %>%
  select(-estimate,-std.error)%>%
  arrange(p.value) %>%
  ungroup()%>%
...
#So to the main question: is the distribution of parameter estimates for each voxel in this ROI different for learners compared to non-learners? No.
hpe_lvstr_regs %>%
...
#is there a difference between the distributions of HPE betas of learner for any of the ROIs? No when looking at adjusted p's. As expected from the GLMs.
...
Looking at means for ease of vieweing (otherwise they are more confusing than helpful since the range of value is not)

More than the mean difference the range of the betas seems interesting.
...
Is the distribution of parameters in an a priori ROI different for learners vs non-learners?

You need to look at each voxel separately

```{r}
all_roi_pe_zvals

value ~ learner+(1|sub_num)
```
