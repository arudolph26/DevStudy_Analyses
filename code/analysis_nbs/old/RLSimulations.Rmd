...
A simple model one could use to analyze the behavior in this task would be a reinforcement learning model (Rescorla-Wagner) with a single learning rate assuming linear utility. Accordingly the probability of playing a machine in a trial is a function of the expected value of that machine in that trial ($EV_t$) and the softmax temperetature ($\beta$)  (the 1 in the denominator is the exponent of the expected value of not playing, i.e. $e^0$)

$p(k_{t} = 1) = \frac{e^{\beta*(EV_t)}}{1+e^{\beta*(EV_t)}}$

where the $EV_t$ is updated after observing the reward ($r$) in each trial at a learning ($\alpha$)
...
Now let's try to recover the parameters. I'm not sure what would be the best starting values so let's try to do this 100 times on the same data with true values for $\alpha$ = 0.05 and $\beta$ = 0.4 with different starting values. The $\beta$ starting values are drawn from a uniform distribution between 0 and 3 (though Daw, 2009 might be suggesting that a gamma distribution might be more appropriate here) and the $\alpha$ starting values are drawn from a uniform distribution between 0 and 1. (Doing this 1000 times can take ~hour locally)

```{r eval=F}
input_df <- data.frame(index = 1:100, beta_start=runif(100, 0, 3), alpha_start=runif(100,0,1))
...
thousand_reps <- input_df %>%
  group_by(index) %>%
  do(run.single.alpha.optim.for.one.row(.,single.alpha.optim,sim_data))
```
...
single.alpha.grid.one.row <- function(row, data){
  ...
  tryCatch(data.frame(neg.log.lik(starting_vals,data)),error = function(e){return(data.frame(NA))}) 
}
...
tmp <- grid_search_df %>%
  group_by(index)%>%
  do(single.alpha.grid.one.row(.,sim_data))
...
ggplot(grid_search_df, aes(beta, alpha))+
  ...
  theme_bw()+
  annotate("point", x = grid_search_df$beta[which(-grid_search_df$neg_log_lik == max(-grid_search_df$neg_log_lik, na.rm = T))], y = grid_search_df$alpha[which(-grid_search_df$neg_log_lik == max(-grid_search_df$neg_log_lik, na.rm=T))], color = 'red', shape=1) +
  annotate("point", x = 0.4, y = 0.05, color='red', shape = 4)+
  scale_fill_continuous(name = "Log likelihood")

grid_search_df %>%
  mutate(neg_log_lik_filter = ifelse(neg_log_lik > 200, 200, neg_log_lik)) %>%
  ggplot(aes(beta, alpha))+
  geom_raster(aes(fill=-neg_log_lik_filter))+
  theme_bw()+
  annotate("point", x = grid_search_df$beta[which(-grid_search_df$neg_log_lik == max(-grid_search_df$neg_log_lik, na.rm = T))], y = grid_search_df$alpha[which(-grid_search_df$neg_log_lik == max(-grid_search_df$neg_log_lik, na.rm=T))], color = 'red', shape=1) +
  annotate("point", x = 0.4, y = 0.05, color='red', shape = 4)+
  scale_fill_continuous(name = "Log likelihood")
...
It seems that if we start $\beta$ from >0 and estimate it first then this method of iterated estimation could recover parameters somewhat successfully. UPDATE: I had written this sentence after estimating parameters on another simulated dataset. The conditions under which it converges on the correct values changes depending on the dataset. There are conditions that work again but they are not the same as before.

To make sure that this isn't a fluke let's try it again on another dataset with different true values.
...
FIX beta starting values to draw from a uniform between 0-3 NOT 1!

Is beta always larger (i.e. does the model always think that the choices are less random) - regenerate the data with the same parameters

Try different parameters (esp for beta since it is close to 0)

Try rewards in same regimes

MLE versus MAP estimates (which requires)

Email Greg (and maybe Nathaniel)
