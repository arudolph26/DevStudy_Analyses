...
This is somewhat counterintuitive and confusing given the assumption in our hypothesis that adults take more risks and therefore perform better in this task. Any increase in 'risk-taking' in this task with age should depend on the machine and its expected value to translate to better performance. (i.e. This graph is confusion; ignore.)
...
Statistical tests on difference in performance: adults different than kids in all but second condition.
...
# Adults make more in low var pos EV machine
tmp <- machine_game_data_clean %>%
  group_by(Sub_id, facet_labels) %>%
  summarise(total_points = sum(Points_earned)) %>%
  do(assign.age.info(.)) %>%
...
# No sig age difference in high var pos EV machine
tmp <- machine_game_data_clean %>%
  group_by(Sub_id, facet_labels) %>%
  summarise(total_points = sum(Points_earned)) %>%
  do(assign.age.info(.)) %>%
...
# Adults loose less in low var neg EV machine
tmp <- machine_game_data_clean %>%
  group_by(Sub_id, facet_labels) %>%
  summarise(total_points = sum(Points_earned)) %>%
  do(assign.age.info(.)) %>%
...
# Adults loose less in high var pos EV machine
tmp <- machine_game_data_clean %>%
  group_by(Sub_id, facet_labels) %>%
  summarise(total_points = sum(Points_earned)) %>%
  do(assign.age.info(.)) %>%
...
summary(lmerTest::lmer(pct_play ~ age_group*facet_labels + (1|Sub_id), data = tmp))
rm(tmp)
```
