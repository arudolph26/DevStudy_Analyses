...
- Adults and teens don't perform better than kids in the positive expected value machines; neither in the low variance positive expected condition (t = 1.717 and t = 1.427 respectively), nor in the high variance condition (t = 0.212 and t = 0.661).      
...
Recoding the behavior in this way gave a clearer picture of the age difference *in learning* between the conditions.
...
**But** it also showed that there is a lot of individual variabilitiy. The multilevel regression below with individual intercepts for each subject regressing the correct choices on age group and condition suggests that close to 40% variance might be due to random effects. It also shows that the fixed effects are not signigicantly different for kids in any condition. 
...
This large between subjects variability suggested that we should look at the behavior at a finer grain instead of just aggregating across age groups. Since we are interested in learning to begin with we look at change in choosing the *optimal* action for a machine across time.  

Here is a plot of change in choosing the correct action for each machine depending on the number of times that machine has been encountered (centered as explained below)  

Things to note for this plot:  
- There is a lot of individual variability in learning (the thin lines in the background are all over the place)  
- Aggregated for age groups it looks like there is no learning in the positive expected value machines (flat lines in the top row). **This seems confusing**  
- Still learning in the high variance positive expected value condition appears lower on aggregate (top row left right comparison)  
- Kids look like they learn best in the low variance negative expected value machine (slope of red line in bottom left quadrant)  
- The age difference in learning appears only in the high variance negative expected value machine (difference between the three line in the bottom right quadrant)  
