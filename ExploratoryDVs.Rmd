...
Subjects completed a probabilistic learning ask in the scanner, a realistic risky decision making task (BART) outside the scanner and numerous questionnaires. The focus of this notebook is on the first two tasks.  
...
Indeed adults earn more points compared to the kids.
...
Since we are interested in the age differences between sensitivity to different feedback schedules, we should show that this difference in performance exists especially for the high variance feedback condition(s).
...
- Kids do better in the low variance conditions compared to the high variance conditions. Kids make more in the low variance positive expected value machine than they they do in the high variance positive expected value machine (t = -2.477). They also loose less in the low variance negative expected value machine (t = -4.409) compared to what they loose in the negative expected value high variance machine (t = - 0.421).  
- Adults and teens don't perform better than kids in the positive expected value machines; neither in the low variance positive expected condition (t = 1.717 and t = 427 respectively), nor in the high variance condition (t = 0.212 and t = 0.661). Though I wonder if it might be harder to detect a performance difference in the high variance positive expected value machine because of the large difference between magnitudes.    
...
The first thing we can look at is how often do subjects play versus pass. It's hard to see any age differences here.  

It is not immediately apparent how to translate this to better performance/learning in this task but one way to think about it: If people learned perfectly they should play half of the time (always for the positive expected value trial and never for the negative expected value trials). The fact that all proportions are above 50% suggests that nobody learns perfectly and that adults might be closest to it. But this is very crude and a better way to look at it would be to see how this depends on the different machines and how it changes throughout the task.
...
To get a better sense of behavior in different contingency states we break this proportion of playing down by machines.

Now we can see age differences in playing in different conditions.
...
```{r}
 machine_game_data_clean %>%
  ...
  do(assign.age.info(.)) %>%
  group_by(age_group, facet_labels) %>%
  summarise(mean_play = mean(pct_play))
```

Let's check which of these behavioral differences in playing are significant.
