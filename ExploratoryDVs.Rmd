...
Since we are interested in differences in learning we must quantify with respect to changes in time.  
...
Should I be making use of the intercept? If I use trials starting at 0 then the intercept is where they start on average (so before learning and not interesting). If I center the trials then the intercept would tell me where they are by the middle of task for each machine. 
...
Note excluding people with too high slopes for depiction.
...
machine_game_data_clean %>%
  ...
  do(get_learning_coef(.)) %>%
  do(assign.age.info(.)) %>%
  # filter(b1<1, learnIndex < 10, learnIndex > -10) %>%
  group_by(age_group, facet_labels) %>%
  summarise(mean_b0 = mean(b0),
            sem_b0 = sem(b0),
            mean_b1 = mean(b1),
            sem_b1 = sem(b1),
            mean_learnIndex = mean(learnIndex),
            sem_learnIndex = sem(learnIndex)) %>%
  gather(key, value, -age_group, -facet_labels) %>%
  separate(key, c("stat", "var")) %>%
  spread(stat, value) %>%
  ggplot(aes(facet_labels, mean, fill = factor(age_group, levels=c('kid', 'teen', 'adult'))))+
  geom_bar(stat='identity', position=position_dodge(0.9))+
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), position = position_dodge(0.9), width=0.25)+
  theme_bw()+
